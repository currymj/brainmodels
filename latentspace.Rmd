...
make_sampler <- function(model) {
    function(sample_num) {
      ...
      if(hasName(model, "hs")) {
        model_list <- list(
          hs=model$hs,
          edgeprobs0=model$edge_probabilities(0L),
          edgeprobs1=model$edge_probabilities(1L)
        )} else {
        model_list <- list(
          edgeprobs0=model$edge_probabilities(0L),
          edgeprobs1=model$edge_probabilities(1L)
...
bernoulli_results <- pblapply(1:n_iters, bernoulli_sampler) %>% map_df(~ .$df) %>% as_tibble
```
```{r}
...
```{r}
standard_models <- map(Ks, ~ lsm$LatentSpaceModel(V, ., sigmasq_b=sigmasq_b, name=paste0("standard", .)))
standard_models %>% walk(model_prep) %>% walk(~ .$initialize)
standard_model_results <- pblapply(standard_models, function(x) {fit_model(x, n_iter=100, progress_bar=FALSE)}, cl=4)
```
...
```{r}
mixture_models <- map(Ks, ~ lsm$MixtureOfLatentSpaceModels(V, .*H, H=H, sigmasq_b=sigmasq_b, name=paste0("mixture", .)))
mixture_models %>% walk(model_prep) %>% walk(~ .$initialize)
mixture_model_results <- pblapply(mixture_models, function(x) {fit_model(x, n_iter=3, progress_bar=FALSE)})
```
